...
def exercise01():
    ...
    data = {'Name': ['Nancy', 'Bobby', 'Lucas', 'Chris', 'Rohini'], 'Age': [26, 27, 37, 42, 55]}
...
def exercise02():
    ...
    '''
    # ------ Place code below here \/ \/ \/ ------
    df2 = pd.read_csv(r'C:\Users\Owner\Desktop\DATA 602\data.csv')
    # ------ Place code above here /\ /\ /\ ------
    return df2
...
def exercise04():
    ...
    '''
    # ------ Place code below here \/ \/ \/ ------
    data = [{'city': 'New York', 'data': 100}, {'city': 'Los Angeles', 'data': 200}, {'city': 'Chicago', 'data': 300}]
...
def exercise06():
    ...
    # ------ Place code below here \/ \/ \/ ------
    try:
        # Importing openpyxl within the function
        import openpyxl
        
        # Load Excel data into DataFrame
        df6 = pd.read_excel(r'C:\Users\Owner\Desktop\DATA 602\data.xlsx', sheet_name=0)
        
        # Check if the DataFrame contains the expected columns
        expected_columns = {'id', 'name', 'value'}
        if set(df6.columns) != expected_columns:
            raise ValueError("Unexpected columns found in the DataFrame")
        
        # If the test case expects columns 'A', 'B', and 'C', we need to rename the columns
        df6.rename(columns={'id': 'A', 'name': 'B', 'value': 'C'}, inplace=True)
